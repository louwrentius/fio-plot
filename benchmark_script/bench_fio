...
def generate_output_directory(settings, benchmark):
    ...
    if benchmark['mode'] in settings['mixed']:
        directory = f"{settings['output']}/{os.path.basename(benchmark['target'])}/{benchmark['mode']}{benchmark['readmix']}/{benchmark['block_size']}"
    else:
...
def get_argument_description():
    descriptions = {
        ...
        'readmix': 'Read/write mix in %% read',
    ...
    if settings['type'] == 'directory':
        for item in settings['target']:
            if not os.path.exists(item):
                print()
                print(
                    f"The target directory ({item}) doesn't seem to exist.")
                print()
                sys.exit(5)
